...
/// Our display with ingridients for pizza
final class IngridientsViewController: UIViewController {
    ...
    @objc private func orderButtonAction() {
        ...
        if chesseSwitch.isOn {
            payViewController.chesseIngridientLabel.text? += "+\(chesseLabel.text ?? "Cheese") "
        }

        if humSwitch.isOn {
            payViewController.chesseIngridientLabel.text? += "+\(hamLabel.text ?? "Sausage") "
        }

        if mushroomsSwitch.isOn {
            payViewController.chesseIngridientLabel.text? += "+\(mushroomsLabel.text ?? "Mushrooms") "
        }

        if maslinsSwitch.isOn {
            payViewController.chesseIngridientLabel.text? += "+\(maslinsLabel.text ?? "Olives") "
        }
